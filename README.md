...
層級會按資料自動壓縮：題名與作者相同的記錄復用作品；版本有可靠識別號時優先按識別號匹配，否則共同按 Edition 題名與副標題、版本名稱、出版社、出版年份和系列判斷，避免不同獨立出版單位被誤合併。卷冊編號與卷冊題名均相同的實物冊歸為同一卷冊組。只有一個版本或卷冊時，界面會省略不必要的下拉層級。

Work 保存題名、副標題、作者或相關責任人及可多選的文種。Edition 保存可選的題名與副標題、識別號、版本、系列、其他題名、翻譯題名、譯者或相關責任人等版本資料。Copy 保存卷冊編號與卷冊題名；識別號不限制 ISBN 格式。
...
首頁提供 JSON 與 CSV 導出按鈕。JSON 保留 Work、Edition、Copy 三層資料，CSV 以每冊實物冊一列輸出，並使用 `edition_title`、`edition_subtitle`、`work_relations`、`volume_number`、`volume_title` 欄位；兩者都包含標籤、藏書位置及閱讀記錄。

CSV 可從首頁導入。系統按題名與作者合併作品，使用與正常新增相同的識別號／出版資訊規則合併版本，再按卷冊編號與卷冊題名提示可能重複的實物冊；每一個疑似重複項都由使用者決定新增另一實物冊，或明確選擇並覆蓋既有 Copy。
