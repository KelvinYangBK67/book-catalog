...
from .schemas import (
    BookInput, BookRecord, CopyInput, EditionInput, PublisherAliasInput,
    PublisherRecord, TagInput, TagRecord, WorkDetail, WorkInput, WorkSummary,
)
...
@app.post("/api/publishers/{publisher_id}/aliases", response_model=PublisherRecord)
def create_publisher_alias(publisher_id: int, payload: PublisherAliasInput) -> dict:
    record = add_publisher_alias(publisher_id, payload)
    if record is None:
        raise HTTPException(status_code=404, detail="找不到此出版社")
    return record
